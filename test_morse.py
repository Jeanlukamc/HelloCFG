...
from morse_to_language import english_equivalent, morse_input_collector
from langauge_to_morse import morse_code_equivalent

@pytest.mark.parametrize( "file, string", [( "FILES\\Morse_Code_Files\\morse_input_grab.txt", '<WHILE> ( ALPHA <SUB> BETA <EQUALS><EQUALS> 15 )' ) ] )
def test_input_collector( file, string ):
    """Tests that we get the proper input to parse"""
    file_input = morse_input_collector( file )
